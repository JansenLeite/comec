# Comunicação Estratégica Corporativa - COMEC
Material de Estudo para o Curso de Técnico em Administração - SENAI Jaguariúna.

<h3>Módulo: 4. Consumidores</h3>

<h4>Conhecimentos Adquiridos:</h4>

<pre>
    <b>4. Consumidores</b>
            4.1. Definição
            4.2. Perfil
            4.3. Canais de atendimento
                4.3.1. Presencial
                4.3.2. Online
            4.4. Coleta e análise de dados
                4.4.1. Ferramentas
                4.4.2. Monitoramento de redes sociais
            4.5. Pesquisas online
</pre>

<h3>Material de Apoio:</h3>

<h4>Introdução a Comsumidores</h4>
<pre><a href="">Clique aqui para acessá-lo.</a></pre>

<h3>1.0 Atividade em Grupo</h3>

<h4>Definições da Atividade</h4>
<pre>
    - Deverá ser criado grupos de 4 a 8 integrantes;
    - Desenvolver um mapa de mapeamento de perfil do cliente baseado no Estudo de Caso entregue pelo professor;
	- Apresentar em Sala de Aula;
</pre>

<h3>Recursos a serem utilizados:</h3>
<pre>
    - Caderno;
    - Caneta;
</pre>